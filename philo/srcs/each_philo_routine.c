...
/*   Updated: 2022/10/15 18:16:02 by nfukuma          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	take_fork_p(t_each_p *each)
{
	...
	util_put_log(each, now, MAGENTA, PIC_FORK);
...
static int	eat_p(t_each_p *each)
{
	...
	util_put_log(each, now, CYAN, PIC_FORK);
	util_put_log(each, now, YELLOW, EATING);
	pthread_mutex_unlock(&each->p_env->printf_mutex_t);
...
static int	sleep_p(t_each_p *each)
{
	...
	util_put_log(each, now, BLUE, SLEEPING);
	pthread_mutex_unlock(&each->p_env->printf_mutex_t);
...
static int	think_p(t_each_p *each)
{
	...
	util_put_log(each, now, WHITE, THINKING);
	pthread_mutex_unlock(&each->p_env->printf_mutex_t);
	return (OTHER_PHILO_ALIVE);
}
